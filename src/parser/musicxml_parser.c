    ...
    switch (n->type) {
        case XML_TAG_OPEN:
        {
            ...
            if (msr_state->curr_chord.deg_count < TMP_CHORD_MAX_DEGREES
                ...
                if (sax_parse_xml(parse_degree, parser_data, context) != 0)
					return PARSER_STOP_ERROR;
            ...
            //     else if (strcmp(alter_str, "-1")) msr_state->curr_chord.root--;
					...
					&& sax_get_content(context, &msr_state->curr_chord.qual) != 0)
					return PARSER_STOP_ERROR; 
    ...
    switch (n->type) {
        case XML_TAG_OPEN:
        {
            if (m->chords.count <= IRP_MAX_CHORDS
                ...
                m->chords.count++;
                if (m->chords.count < IRP_MAX_CHORDS
						&& sax_parse_xml(parse_harmony, parser_data, context) != 0)
                	return PARSER_STOP_ERROR;
            } else if (str_ref_eq(&n->target, &musicxml.barline)
				   	&& sax_parse_xml(parse_barline, parser_data, context) != 0) { 
					return PARSER_STOP_ERROR;
            } else {
                return PARSER_CONTINUE | SKIP_ENTIRE_NODE;
			}
...
static int parse_work(void *user_data, sax_context *context)
{
    ...
    switch (n->type) {
        case XML_TAG_OPEN:
        {
            if (!parser_data->song->title[0] && str_ref_eq(&n->target, &musicxml.work_title)) { 
                if (sax_copy_content(context, parser_data->song->title, IRP_MAX_CREDENTIALS) != 0)
					return PARSER_STOP_ERROR;
    ...
    switch (n->type) {
        case XML_TAG_OPEN:
        {
            ...
            if (!parser_data->song->composer[0]
                ...
                if (sax_copy_content(context, parser_data->song->composer, IRP_MAX_CREDENTIALS) != 0)
					return PARSER_STOP_ERROR;
                // we have what we need, so we skip the rest of parent content
...
static int parse_song_partwise(void *user_data, sax_context *context)
{
    ...
    if (n->type == XML_TAG_OPEN) {
        ...
        if (str_ref_eq(&n->target, &musicxml.work)
            ...
            if (sax_parse_xml(parse_part, parser_data, context) != 0)
					return PARSER_STOP_ERROR;
            return PARSER_STOP;
        } else {
            return PARSER_CONTINUE | SKIP_ENTIRE_NODE;
